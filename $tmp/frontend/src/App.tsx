...
import { Plus, Users, LayoutGrid, ClipboardList, Globe } from 'lucide-react';
...
function Layout() {
  ...
  const hideNav = location.pathname === '/create-order' || location.pathname === '/login' || location.pathname.startsWith('/orders/edit/') || location.pathname === '/attach-barcode' || location.pathname === '/online-orders';
      ...
      <div className="w-full max-w-md min-h-screen bg-slate-50 relative shadow-2xl overflow-x-hidden border-x border-slate-300">
        ...
        <main className={`relative z-10 w-full h-full min-h-screen ${!hideNav ? 'pb-24' : ''}`}>
          <Routes>
            ...
            <Route path="/online-orders" element={<AuthGuard><OnlineOrder /></AuthGuard>} />
          </Routes>
        </main>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50">
          <nav className="bg-white/95 backdrop-blur-xl border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] pb-safe">
            ...
            <div className={`grid ${isAdmin ? 'grid-cols-4' : 'grid-cols-2'} h-16 max-w-md mx-auto`}>
               ...
               {isAdmin && (
                 <NavLink to="/online-orders" icon={Globe} label="Online" />
               )}
